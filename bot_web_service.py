...
# Configurar logging mejorado con más niveles de detalle
logging.basicConfig(
    level=logging.INFO, 
    stream=sys.stdout, 
            ...
            with open(self.log_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    try:
                        ...
                        logger.warning(f"⚠ Error procesando fila en datos: {e}")
                        continue
...
class BitgetClient:
    def __init__(self, api_key, api_secret, passphrase):
        ...
        self.product_types = ['USDT-FUTURES', 'USDT-MIX']  # Soporte para ambos tipos

    def _generate_signature(self, timestamp, method, request_path, body=''):
        """Generar firma HMAC-SHA256 corregida para Bitget V2"""
        try:
            ...
            # Crear el mensaje para firmar: timestamp + method + request_path + body
            message = timestamp + method.upper() + request_path + body_str
            
            logger.debug(f"🔐 Generando firma para: {method} {request_path}")
            logger.debug(f"📝 Mensaje a firmar: '{message}'")
    ...
    def verificar_credenciales(self):
        """Verificar que las credenciales sean válidas con manejo mejorado de errores"""
        try:
            ...
            # Probar con ambos productTypes
            for product_type in self.product_types:
                try:
                    logger.info(f"🔄 Probando con productType: {product_type}")
                    ...
                    if accounts:
                        ...
                        logger.warning(f"⚠ No se obtuvo información con {product_type}")
                except Exception as e:
                    logger.warning(f"⚠ Error con {product_type}: {e}")
                    continue
    ...
    def get_account_info(self, product_type='USDT-FUTURES'):
        """Obtener información de cuenta Bitget V2 con manejo de errores mejorado"""
        try:
            ...
            logger.debug(f"📡 Request: GET {request_path} con params: {params}")
            ...
            if response.status_code == 200:
                ...
                if data.get('code') == '00000':
                    logger.info(f"✅ Éxito con productType: {product_type}")
                    return data.get('data', [])
                else:
                    error_msg = data.get('msg', 'Unknown error')
                    error_code = data.get('code', 'Unknown')
                    logger.error(f"❌ Error API {product_type}: {error_code} - {error_msg}")
                    
                    # Manejo específico de errores conocidos
                    if error_code == '40020':
                        logger.warning(f"⚠ productType {product_type} no válido, probando alternativo...")
                        return None
                    elif error_code == '40009':
                        logger.error("❌ Error de firma - verificar credentials y timestamp")
                        return None
                    elif error_code == '40001':
                        logger.error("❌ API Key inválida")
                        return None
                    elif error_code == '40002':
                        logger.error("❌ Secret Key inválida")
                        return None
            else:
    ...
    def get_symbol_info(self, symbol):
        """Obtener información del símbolo con manejo de errores"""
        try:
                        ...
                        if data.get('code') == '00000':
                            ...
                            for contract in contracts:
                                if contract.get('symbol') == symbol:
                                    logger.info(f"✅ Info de {symbol} encontrada con {product_type}")
                                    return contract
                    else:
                        logger.warning(f"⚠ Error obteniendo contratos con {product_type}: {response.status_code}")
                except Exception as e:
                    logger.warning(f"⚠ Error con {product_type}: {e}")
                    continue
            
            logger.warning(f"⚠ No se encontró información para {symbol}")
            return None
    ...
    def place_order(self, symbol, side, order_type, size, price=None, 
        ...
        """Colocar orden de mercado o límite con manejo de errores mejorado"""
        try:
            ...
            for product_type in self.product_types:
                try:
                    ...
                    logger.debug(f"📤 Enviando orden: {body}")
                    ...
                    if response.status_code == 200:
                        data = response.json()
                        logger.debug(f"📋 Respuesta orden: {data}")
                        
                        if data.get('code') == '00000':
                            logger.info(f"✅ Orden colocada exitosamente con {product_type}")
                            return data.get('data', {})
                        else:
                            error_msg = data.get('msg', 'Unknown error')
                            error_code = data.get('code', 'Unknown')
                            logger.error(f"❌ Error en orden con {product_type}: {error_code} - {error_msg}")
                            
                            # Si es error de productType, continuar con el siguiente
                            if error_code == '40020':
                                logger.warning(f"⚠ {product_type} no válido para orden, probando siguiente...")
                                continue
                    else:
                        logger.error(f"❌ Error HTTP orden: {response.status_code} - {response.text}")
                except Exception as e:
                    logger.warning(f"⚠ Error con {product_type}: {e}")
                    continue
            
            logger.error(f"❌ No se pudo colocar orden para {symbol} con ningún productType")
            return None
    ...
    def place_plan_order(self, symbol, side, trigger_price, order_type, size, 
        ...
        """Colocar orden de plan (TP/SL) con manejo de errores"""
        try:
            ...
            for product_type in self.product_types:
                try:
                    ...
                    logger.debug(f"📤 Enviando plan order: {body}")
                    ...
                    if response.status_code == 200:
                        data = response.json()
                        if data.get('code') == '00000':
                            logger.info(f"✅ Plan order colocada con {product_type}")
                            return data.get('data', {})
                        else:
                            error_code = data.get('code', 'Unknown')
                            error_msg = data.get('msg', 'Unknown error')
                            logger.error(f"❌ Error plan order {product_type}: {error_code} - {error_msg}")
                            
                            if error_code == '40020':
                                continue
                    else:
                        logger.error(f"❌ HTTP Error plan order: {response.status_code}")
                except Exception as e:
                    logger.warning(f"⚠ Error plan order con {product_type}: {e}")
                    continue
            
            logger.warning(f"⚠ No se pudo colocar plan order para {symbol}")
            return None
    ...
    def set_margin_mode(self, symbol, margin_mode='isolated'):
        """Configurar modo de margen con manejo de errores mejorado"""
        try:
            ...
            for product_type in self.product_types:
                try:
                    ...
                    if response.status_code == 200:
                        data = response.json()
                        if data.get('code') == '00000':
                            logger.info(f"✅ Modo margen {margin_mode} configurado para {symbol} con {product_type}")
                            return True
                        # Si ya está en ese modo, también es éxito
                        elif data.get('code') == '40756':
                            logger.info(f"✅ Margen {margin_mode} ya estaba configurado para {symbol}")
                            return True
                        else:
                            logger.warning(f"⚠ Error configurando margin mode con {product_type}: {data}")
                    else:
                        logger.warning(f"⚠ HTTP Error margin mode: {response.status_code}")
                except Exception as e:
                    logger.warning(f"⚠ Error margin mode con {product_type}: {e}")
                    continue
            
            logger.warning(f"⚠ No se pudo configurar margin mode para {symbol}")
    ...
    def set_leverage(self, symbol, leverage, hold_side='long', margin_type='isolated'):
        """Configurar apalancamiento con manejo de errores mejorado"""
        try:
            logger.info(f"⚙️ Configurando leverage {leverage}x para {symbol} con {hold_side}")
            
            # Primero configurar el modo de margen
            margin_ok = self.set_margin_mode(symbol, margin_type)
            if not margin_ok:
                logger.warning("⚠ Error configurando margen, continuando...")
            time.sleep(0.5)
            ...
            for product_type in self.product_types:
                try:
                    ...
                    if response.status_code == 200:
                        data = response.json()
                        if data.get('code') == '00000':
                            logger.info(f"✅ Apalancamiento {leverage}x configurado para {symbol} con {product_type}")
                            return True
                        # Si el leverage ya está configurado, también es éxito
                        elif data.get('code') == '40761':
                            logger.info(f"✅ Apalancamiento {leverage}x ya estaba configurado para {symbol}")
                            return True
                        else:
                            error_msg = data.get('msg', 'Unknown error')
                            logger.warning(f"⚠ Error configurando leverage con {product_type}: {error_msg}")
                    else:
                        logger.warning(f"⚠ HTTP Error leverage: {response.status_code}")
                except Exception as e:
                    logger.warning(f"⚠ Error leverage con {product_type}: {e}")
                    continue
            
            logger.warning(f"⚠ No se pudo configurar leverage para {symbol}")
            return False
    ...
    def get_positions(self, symbol=None, product_type='USDT-FUTURES'):
        """Obtener posiciones abiertas con manejo de errores mejorado"""
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('code') == '00000':
                    ...
                    logger.warning(f"⚠ Error obteniendo posiciones con {product_type}: {data}")
            
            # Si falla USDT-FUTURES, probar con USDT-MIX
            if product_type == 'USDT-FUTURES':
                logger.info("🔄 Probando USDT-MIX para posiciones...")
                return self.get_positions(symbol, 'USDT-MIX')
            
            return []
            
        except Exception as e:
            logger.error(f"❌ Error obteniendo posiciones para {symbol}: {e}")
            return []

    def get_klines(self, symbol, interval='5m', limit=200):
        """Obtener velas (datos de mercado) con manejo de errores mejorado"""
        try:
            interval_map = {
                '1m': '1m', '3m': '3m', '5m': '5m',
            ...
            for product_type in self.product_types:
                try:
                    ...
                    logger.debug(f"📡 Obteniendo klines para {symbol} con {product_type}")
                    ...
                    if response.status_code == 200:
                        ...
                        if data.get('code') == '00000':
                            ...
                            logger.debug(f"✅ Obtenidas {len(candles)} velas para {symbol}")
                            return candles
                        else:
                            logger.warning(f"⚠ Error klines con {product_type}: {data}")
                    else:
                        logger.warning(f"⚠ HTTP Error klines: {response.status_code}")
                except Exception as e:
                    logger.warning(f"⚠ Error klines con {product_type}: {e}")
                    continue
...
# ---------------------------
# FUNCIÓN DE PRUEBA PARA VERIFICAR OPERACIONES EN BITGET
# ---------------------------
def probar_conexion_bitget(bitget_client, simbolo='BTCUSDT'):
    """
    Función para probar que el código funciona correctamente con Bitget
    Ejecuta una serie de verificaciones y pruebas básicas
    """
    ...
    try:
        ...
        logger.info("2️⃣ Obteniendo información de cuenta...")
        ...
        logger.info(f"3️⃣ Obteniendo información de {simbolo}...")
        symbol_info = bitget_client.get_symbol_info(simbolo)
        if not symbol_info:
            logger.error(f"❌ No se pudo obtener información de {simbolo}")
            return False
        logger.info(f"✅ Info de {simbolo} obtenida")
        ...
        logger.info("4️⃣ Obteniendo datos de mercado...")
        klines = bitget_client.get_klines(simbolo, '1m', 10)
        if not klines:
        ...
        # 5. Verificar configuración de margen
        logger.info("5️⃣ Verificando configuración de margen...")
        margin_ok = bitget_client.set_margin_mode(simbolo, 'isolated')
        if margin_ok:
            logger.info("✅ Configuración de margen verificada")
        else:
            logger.warning("⚠ Error en configuración de margen")
        
        # 6. Verificar configuración de leverage
        logger.info("6️⃣ Verificando configuración de leverage...")
        leverage_ok = bitget_client.set_leverage(simbolo, 10, 'long', 'isolated')
        if leverage_ok:
            logger.info("✅ Configuración de leverage verificada")
        else:
            logger.warning("⚠ Error en configuración de leverage")
        
        # 7. Obtener posiciones actuales
        logger.info("7️⃣ Obteniendo posiciones actuales...")
        posiciones = bitget_client.get_positions(simbolo)
        logger.info(f"✅ Posiciones obtenidas: {len(posiciones)}")
        
        logger.info("=" * 50)
        logger.info("🎉 TODAS LAS PRUEBAS COMPLETADAS EXITOSAMENTE")
        logger.info("✅ El bot está listo para operar en Bitget")
        return True
...
def ejecutar_operacion_bitget(bitget_client, simbolo, tipo_operacion, capital_usd, leverage=20):
    ...
    try:
        ...
        logger.info(f"⚙️ Configurando leverage para {hold_side}")
        leverage_ok = bitget_client.set_leverage(simbolo, leverage,_type='isolated hold_side, margin')
        if not leverage_ok:
...
class TradingBot:
        ...
        if config.get('bitget_api_key') and config.get('bitget_api_secret') and config.get('bitget_passphrase'):
            try:
                ...
                if probar_conexion_bitget(self.bitget_client):
                    ...
                    logger.warning("⚠️ Pruebas de conexión fallidas")
            except Exception as e:
                            ...
                            if datos:
                                ...
                                if token and chats:
                                    try:
                                        ...
                                        logger.warning(f"⚠ Error enviando mensaje de cierre: {e}")
    ...
    def obtener_datos_mercado_config(self, simbolo, timeframe, num_velas):
        """Obtener datos de mercado para configuración específica"""
        try:
            if self.bitget_client:
                # Usar Bitget API
                klines = self.bitget_client.get_klines(simbolo, timeframe, num_velas)
                if not klines:
                    logger.warning(f"⚠ No se pudieron obtener datos de Bitget para {simbolo}")
                    return self._fallback_binance_datos(simbolo, timeframe, num_velas)
                ...
                # Bitget devuelve datos en formato [timestamp, open, high, low, close, volume]
                for kline in klines:
                    timestamp = int(kline[0])
                    apertura = float(kline[1])
                    maximo = float(kline[2])
                    minimo = float(kline[3])
                    cierre = float(kline[4])
                    volumen = float(kline[5])
                    
                    datos['aperturas'].append(apertura)
                    datos['maximos'].append(maximo)
                    datos['minimos'].append(minimo)
                    datos['cierres'].append(cierre)
                    datos['volumenes'].append(volumen)
                    datos['tiempos'].append(timestamp)
                
                datos['precio_actual'] = datos['cierres'][-1] if datos['cierres'] else 0
                return datos
            else:
                # Fallback a Binance
                return self._fallback_binance_datos(simbolo, timeframe, num_velas)
    ...
    def _fallback_binance_datos(self, simbolo, timeframe, num_velas):
    ...
    def calcular_canal_regresion_config(self, datos_mercado, num_velas):
        """Calcular canal de regresión para configuración específica"""
        try:
            cierres = datos_mercado['cierres']
            if len(cierres) < num_velas:
                return None
            
            # Usar las últimas velas
            ...
            pearson, angulo_grados = self.calcular_pearson_y_angulo(x, cierres_recientes)
            ...
            maximo = max(datos_mercado['maximos'][-num_velas:])
            minimo = min(datos_mercado['minimos'][-num_velas:])
            ancho_canal = maximo - minimo
            precio_actual = datos_mercado['precio_actual']
            ancho_canal_porcentual = (ancho_canal / precio_actual) * 100 if precio_actual > 0 else 0
            
            # Calcular pendientes de resistencia y soporte
            # Resistencia: línea de tendencia superior
            resistencia_y = []
            soporte_y = []
            
            for i, precio in enumerate(cierres_recientes):
                # Línea central
                linea_central = pendiente * i + intercepto
                
                # Calcular desviación estándar de los residuos
                residuos = [cierres_recientes[j] - (pendiente * j + intercepto) for j in range(len(cierres_recientes))]
                std_residuos = np.std(residuos) if len(residuos) > 1 else 0
                
                # Resistencias y soportes
                resistencia = linea_central + 2 * std_residuos
                soporte = linea_central - 2 * std_residuos
                
                resistencia_y.append(resistencia)
                soporte_y.append(soporte)
            
            # Pendientes de resistencia y soporte
            pendiente_resistencia, _ = self.calcular_regresion_lineal(x, resistencia_y)
            pendiente_soporte, _ = self.calcular_regresion_lineal(x, soporte_y)
            ...
            logger.error(f"❌ Error calculando canal para datos: {e}")
            return None

    def detectar_breakout(self, simbolo, info_canal, datos_mercado):
        """
        Detectar breakout según la lógica:
        - BREAKOUT_LONG: Ruptura de soporte en canal bajista
        - BREAKOUT_SHORT: Ruptura de resistencia en canal alcista
        """
        try:
            precio_actual = datos_mercado['precio_actual']
            ...
            # Solo detectar breakouts en canales válidos
            if info_canal['nivel_fuerza'] < 2 or abs(info_canal['coeficiente_pearson']) < 0.4:
                return None
            
            # Lógica de breakout CORREGIDA:
            # - BREAKOUT_LONG: Canal bajista (dirección = BAJISTA) + precio rompe soporte hacia abajo
            # - BREAKOUT_SHORT: Canal alcista (dirección = ALCISTA) + precio rompe resistencia hacia arriba
            
            if direccion == "🔴 BAJISTA" and precio_actual <= soporte:
                # Canal bajista rompe soporte → Esperamos reentry LONG
                logger.info(f"📈 {simbolo} - BREAKOUT_LONG detectado (canal bajista rompe soporte)")
                return "BREAKOUT_LONG"
            elif direccion == "🟢 ALCISTA" and precio_actual >= resistencia:
                # Canal alcista rompe resistencia → Esperamos reentry SHORT  
                logger.info(f"📉 {simbolo} - BREAKOUT_SHORT detectado (canal alcista rompe resistencia)")
    ...
    def detectar_reentry(self, simbolo, info_canal, datos_mercado):
        """
        Detectar reentry según la lógica:
        - Si esperamos BREAKOUT_LONG → Señal SHORT (comprar en soporte roto)
        - Si esperamos BREAKOUT_SHORT → Señal LONG (vender en resistencia rota)
        """
        try:
            ...
            # Verificar que el precio volvió al canal
            if precio_actual <= resistencia and precio_actual >= soporte:
                # Verificar confirmación con Stochastic
                stoch_k = info_canal['stoch_k']
                stoch_d = info_canal['stoch_d']
                
                if tipo_breakout == "BREAKOUT_LONG":
                    # Esperamos señal SHORT cuando el precio vuelve al soporte roto
                    if stoch_k <= 30 and stoch_d <= 30:
                        logger.info(f"📉 {simbolo} - Señal SHORT confirmada (reentry en soporte con oversold)")
                        return "SHORT"
                        
                elif tipo_breakout == "BREAKOUT_SHORT":
                    # Esperamos señal LONG cuando el precio vuelve a la resistencia rota
                    if stoch_k >= 70 and stoch_d >= 70:
                        logger.info(f"📈 {simbolo} - Señal LONG confirmada (reentry en resistencia con overbought)")
                        return "LONG"
    ...
    def calcular_niveles_entrada(self, tipo_operacion, info_canal, precio_actual):
        """Calcular niveles de entrada con validación de RR"""
        try:
            # Usar márgenes de entrada configurados
            entry_margin = self.config.get('entry_margin', 0.001)
            
            if tipo_operacion == "LONG":
                # Entrada cerca del soporte con margen
                precio_entrada = info_canal['soporte'] * (1 + entry_margin)
                
                # Stop loss: debajo del soporte
                stop_loss = info_canal['soporte'] * 0.98
                
                # Take profit: basado en la resistencia
                riesgo = abs(precio_entrada - stop_loss)
                take_profit = precio_entrada + (riesgo * 2)  # RR 2:1
                
            else:  # SHORT
                # Entrada cerca de la resistencia con margen
                precio_entrada = info_canal['resistencia'] * (1 - entry_margin)
                
                # Stop loss: encima de la resistencia
                stop_loss = info_canal['resistencia'] * 1.02
                
                # Take profit: basado en el soporte
                riesgo = abs(stop_loss - precio_entrada)
                take_profit = precio_entrada - (riesgo * 2)  # RR 2:1
            
            # Validar ratio RR mínimo
            riesgo = abs(precio_entrada - stop_loss)
            ...
            logger.error(f"❌ Error calculando niveles para {tipo_operacion}: {e}")
    ...
    def enviar_alerta_breakout(self, simbolo, tipo_breakout, info_canal, datos_mercado, config_optima):
        """Enviar alerta de breakout detectado a Telegram"""
        try:
            precio_actual = datos_mercado['precio_actual']
            mensaje = f"""
...
📏 <b>Canal:</b> {info_canal['direccion']} ({info_canal['angulo_tendencia']:.1f}°)
📊 <b>Ancho:</b> {info_canal['ancho_canal_porcentual']:.1f}%
...
🕒 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
            ...
            logger.warning(f"⚠ Error enviando alerta de breakout: {e}")
    ...
    def generar_senal_operacion(self, simbolo, tipo_operacion, precio_entrada, tp, sl,
        ...
        if precio_entrada is None or tp is None or sl is None:
            logger.debug(f"    ❌ Niveles inválidos para {simbolo}, omitiendo señal")
            return
    ...
    def _enviar_telegram_simple(self, mensaje, token, chat_ids):
        ...
        for chat_id in chat_ids:
            ...
            try:
                ...
                logger.warning(f"⚠️ Error enviando mensaje a {chat_id}: {e}")
    ...
    def mostrar_resumen_operaciones(self):
        ...
        if self.bitget_client:
            ...
            try:
                ...
                logger.info(f"   [POSITIONS] Posiciones Reales en Bitget: {len(posiciones_abiertas)}")
                ...
                if posiciones_abiertas:
                    for pos in posiciones_abiertas:
                        ...
                        logger.info(f"   • {simbolo} {tipo} | PnL: {pnl:.2f} USDT")
                        
            except Exception as e:
                logger.warning(f"   ⚠️ Error consultando posiciones reales: {e}")
        else:
...
def telegram_webhook():
    if request.is_json:
        ...
        logger.info(f"Update recibido: {json.dumps(update)}", file=sys.stdout)
...
# Endpoint de prueba para verificar operaciones Bitget
@app.route('/test_bitget', methods=['POST'])
def test_bitget_operation():
    """Endpoint para probar operaciones en Bitget"""
    try:
        if not bot.bitget_client:
            return jsonify({"error": "Cliente Bitget no configurado"}), 400
        
        data = request.get_json() or {}
        simbolo = data.get('symbol', 'BTCUSDT')
        tipo_operacion = data.get('type', 'LONG')
        capital_usd = data.get('capital', 2)
        leverage = data.get('leverage', 10)
        
        logger.info(f"🧪 Ejecutando prueba de operación en Bitget: {simbolo} {tipo_operacion}")
        
        # Ejecutar la operación de prueba
        resultado = ejecutar_operacion_bitget(
            bitget_client=bot.bitget_client,
            simbolo=simbolo,
            tipo_operacion=tipo_operacion,
        ...
        if resultado:
            ...
            return jsonify({
                "success": False,
                "error": "Error ejecutando operación"
            }), 400
            
    except Exception as e:
        logger.error(f"❌ Error en test_bitget_operation: {e}")
        return jsonify({"error": str(e)}), 500

# Endpoint de diagnóstico completo
@app.route('/diagnostic', methods=['GET'])
def diagnostic_bitget():
    """Endpoint para diagnóstico completo de Bitget"""
    try:
        if not bot.bitget_client:
            return jsonify({"error": "Cliente Bitget no configurado"}), 400
        
        logger.info("🔍 Ejecutando diagnóstico completo de Bitget...")
        
        # Ejecutar pruebas de conexión
        pruebas_exitosas = probar_conexion_bitget(bot.bitget_client)
        
        return jsonify({
            "success": pruebas_exitosas,
            "timestamp": datetime.now().isoformat(),
            "config": {
                "api_key": bot.bitget_client.api_key[:10] + "..." if bot.bitget_client.api_key else None,
                "base_url": bot.bitget_client.base_url,
                "product_types": bot.bitget_client.product_types
            },
            "message": "Diagnóstico completado" if pruebas_exitosas else "Se encontraron problemas en las pruebas"
        }), 200
        
    except Exception as e:
        logger.error(f"❌ Error en diagnostic: {e}")
        return jsonify({"error": str(e)}), 500

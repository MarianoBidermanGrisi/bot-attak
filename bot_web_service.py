...
class BitgetClient:
    ...
    def _generate_signature(self, timestamp, method, request_path, body=''):
        ...
        try:
            ...
            logger.debug(f"Firma generada para: {message}")
    ...
    def _get_headers(self, method, request_path, body=''):
        """Obtener headers con firma para Bitget V2 - MEJORADO"""
        try:
    ...
    def _make_request(self, method, endpoint, params=None, data=None, timeout=30):
        """Método unificado para hacer requests con reintentos y mejor logging"""
        max_retries = 3
        retry_delay = 1
        ...
        for attempt in range(max_retries):
            try:
                ...
                if method.upper() == 'GET':
                    ...
                    if data:
                        kwargs['json'] = data
                        # Generar headers para POST
                        body_str = json.dumps(data, separators=(',', ':')) if data else ''
                        endpoint_with_query = endpoint
                        if params:
                            query_parts = [f"{k}={v}" for k, v in params.items()]
                            endpoint_with_query += "?" + "&".join(query_parts)
                        kwargs['headers'] = self._get_headers('POST', endpoint_with_query, data)
                    else:
                        kwargs['headers'] = self._get_headers('POST', endpoint, '')
                ...
                if response.status_code == 200:
                    ...
                    if attempt < max_retries - 1:
                        logger.warning(f"🔄 Reintentando en {retry_delay} segundos (intento {attempt + 1}/{max_retries})")
                ...
                logger.error(f"⏰ Timeout en intento {attempt + 1}: {e}")
                if attempt < max_retries - 1:
                    logger.warning(f"🔄 Reintentando en {retry_delay} segundos...")
                    time.sleep(retry_delay)
                    retry_delay *= 2
    ...
    def verificar_credenciales(self):
        ...
        try:
            ...
            if not self.api_key or not self.api_secret or not self.passphrase:
                logger.error("❌ Credenciales incompletas - Faltan API_KEY, SECRET_KEY o PASSPHRASE")
                return False
            
            logger.info(f"🔑 API Key: {self.api_key[:10]}...")
            logger.info(f"🔒 Secret Key: {self.api_secret[:10]}...")
            logger.info(f"🗝️ Passphrase: {self.passphrase}")
            
            # Intentar obtener información de cuenta con diferentes productTypes
            product_types = ['USDT-FUTURES', 'USDT-MIX', 'USDT-FUNDING']
            
            for product_type in product_types:
                logger.info(f"📊 Intentando con productType: {product_type}")
                accounts = self.get_account_info(product_type)
                if accounts:
                    logger.info(f"✅ Credenciales verificadas exitosamente con {product_type}")
                    ...
                    logger.warning(f"⚠️ No se pudo obtener cuenta con {product_type}")
    ...
    def get_account_info(self, product_type='USDT-FUTURES'):
        ...
        try:
            ...
            logger.info(f"🔍 Consultando cuenta con productType: {product_type}")
            
            response = self._make_request('GET', request_path, params=params)
            
            if response and response.status_code == 200:
                data = response.json()
                logger.info(f"📋 Respuesta de cuenta: {data}")
                ...
                logger.error(f"❌ No se recibió respuesta válida")
                return None
    ...
    def get_symbol_info(self, symbol):
        """Obtener información del símbolo - MEJORADO"""
        try:
            request_path = '/api/v2/mix/market/contracts'
            
            # Intentar con diferentes productTypes
                    ...
                    if data.get('code') == '00000':
                        ...
                        for contract in contracts:
                            if contract.get('symbol') == symbol:
                                logger.info(f"✅ Símbolo {symbol} encontrado en {product_type}")
                                return contract
                        logger.info(f"ℹ️ Símbolo {symbol} no encontrado en {product_type}")
                    else:
                        logger.error(f"❌ Error obteniendo contratos: {data}")
    ...
    def place_order(self, symbol, side, order_type, size, price=None, 
        ...
        """Colocar orden de mercado o límite con margen aislado - MEJORADO"""
        try:
            request_path = '/api/v2/mix/order/place-order'
            
            # Verificar productType basado en el símbolo
            ...
            if symbol.endswith('USDT'):
                ...
                product_type = 'USDT-FUTURES'  # Default
            ...
            logger.info(f"📤 Colocando orden: {json.dumps(body, indent=2)}")
            
            response = self._make_request('POST', request_path, data=body)
            
            if response and response.status_code == 200:
                data = response.json()
                logger.info(f"📥 Respuesta de orden: {data}")
                ...
                logger.error(f"❌ Error HTTP colocando orden: {response.status_code if response else 'No response'}")
                return None
    ...
    def place_plan_order(self, symbol, side, trigger_price, order_type, size, 
        ...
        """Colocar orden de plan (TP/SL) con margen aislado - MEJORADO"""
        try:
            ...
            logger.info(f"📤 Colocando plan order: {json.dumps(body, indent=2)}")
            
            response = self._make_request('POST', request_path, data=body)
            
            if response and response.status_code == 200:
                data = response.json()
                logger.info(f"📥 Respuesta de plan order: {data}")
                
                if data.get('code') == '00000':
                    logger.info(f"✅ Plan order colocado exitosamente")
                    return data.get('data', {})
                else:
                    error_msg = data.get('msg', 'Unknown error')
                    error_code = data.get('code', 'Unknown')
                ...
                logger.error(f"❌ Error HTTP colocando plan order: {response.status_code if response else 'No response'}")
                return None
    ...
    def set_margin_mode(self, symbol, margin_mode='isolated'):
        """Configurar modo de margen (isolated o crossed) - MEJORADO"""
        try:
            ...
            logger.info(f"📤 Configurando margin mode: {json.dumps(body, indent=2)}")
            
            response = self._make_request('POST', request_path, data=body)
            
            if response and response.status_code == 200:
                data = response.json()
                logger.info(f"📥 Respuesta de margin mode: {data}")
                ...
                logger.warning(f"⚠️ Error configurando margin mode: {data}")
                return True  # Continuar aunque falle
            else:
                logger.error(f"❌ Error HTTP configurando margin mode: {response.status_code if response else 'No response'}")
                return True  # Continuar aunque falle
    ...
    def set_leverage(self, symbol, leverage, hold_side='long', margin_type='isolated'):
        """Configurar apalancamiento con margen aislado - MEJORADO"""
        try:
            # Primero configurar el modo de margen
            logger.info(f"⚙️ Configurando margen para {symbol}...")
            margin_ok = self.set_margin_mode(symbol, margin_type)
            if not margin_ok:
                logger.warning("⚠️ Error configurando modo de margen, continuando...")
            time.sleep(0.5)
            ...
            logger.info(f"📤 Configurando leverage: {json.dumps(body, indent=2)}")
            
            response = self._make_request('POST', request_path, data=body)
            
            if response and response.status_code == 200:
                data = response.json()
                logger.info(f"📥 Respuesta de leverage: {data}")
                ...
                logger.error(f"❌ Error HTTP configurando leverage: {response.status_code if response else 'No response'}")
                return False
    ...
    def get_positions(self, symbol=None, product_type='USDT-FUTURES'):
        ...
        try:
            ...
            if response and response.status_code == 200:
                data = response.json()
                logger.info(f"📥 Respuesta de posiciones: {data}")
                ...
                if data.get('code') == '00000':
                    ...
                    error_msg = data.get('msg', 'Unknown error')
                    error_code = data.get('code', 'Unknown')
                    logger.error(f"❌ Error obteniendo posiciones: {error_code} - {error_msg}")
            ...
            logger.error(f"❌ No se pudieron obtener posiciones")
            return []
    ...
    def get_klines(self, symbol, interval='5m', limit=200):
        ...
        try:
            ...
            logger.info(f"🔍 Obteniendo klines para {symbol} - {interval}")
            
            response = self._make_request('GET', request_path, params=params, timeout=15)
            
            if response and response.status_code == 200:
                data = response.json()
                logger.info(f"📥 Respuesta de klines: {len(data.get('data', []))} velas")
                
                if data.get('code') == '00000':
                    candles = data.get('data', [])
                    logger.info(f"✅ Obtenidas {len(candles)} velas para {symbol}")
                    return candles
                else:
                    error_msg = data.get('msg', 'Unknown error')
                    error_code = data.get('code', 'Unknown')
                    logger.error(f"❌ Error obteniendo klines: {error_code} - {error_msg}")
                    ...
                    if product_type == 'USDT-FUTURES':
                        ...
                        if response and response.status_code == 200:
                            ...
                            if data.get('code') == '00000':
                                candles = data.get('data', [])
                                logger.info(f"✅ Obtenidas {len(candles)} velas para {symbol} con USDT-MIX")
                                return candles
...
def ejecutar_operacion_bitget(bitget_client, simbolo, tipo_operacion, capital_usd, leverage=20):
    ...
    try:
        ...
        logger.info(f"⚙️ Configurando leverage {leverage}x para {simbolo}...")
        ...
        logger.info(f"📤 Abriendo posición {side} para {simbolo}...")
        ...
        logger.info(f"🛑 Configurando Stop Loss para {simbolo}...")
        ...
        logger.info(f"🎯 Configurando Take Profit para {simbolo}...")
...
class TradingBot:
    def __init__(self, config):
        ...
        logger.info(f"   - Ejecutar operaciones automáticas: {self.ejecutar_operaciones_automaticas}")
    ...
    def sincronizar_estado_con_bitget(self):
        ...
        try:
            ...
            logger.info(f"   - Operaciones eliminadas: {len(simbolos_a_eliminar)}")
            logger.info(f"   - Operaciones agregadas: {len([p for p in posiciones_reales if p.get('positionSize', 0) != 0 and p.get('symbol') not in simbolos_a_eliminar])}")
    ...
    def verificar_operaciones_reales_bitget(self):
        """Verifica el cierre de operaciones consultando directamente Bitget"""
        if not self.bitget_client:
            return []
        ...
        try:
            ...
            posiciones_reales = self.bitget_client.get_positions()
            simbolos_en_bitget = {pos.get('symbol') for pos in posiciones_reales if pos.get('positionSize', 0) != 0}
            
            # Verificar cada operación en nuestro estado interno
            for simbolo in list(self.operaciones_activas.keys()):
                if simbolo not in simbolos_en_bitget:
                    # La posición fue cerrada en Bitget
                    logger.info(f"🔍 Detectando cierre real en Bitget para {simbolo}")
                    
                    # Buscar la posición cerrada en la lista completa
                    for pos in posiciones_reales:
                        if pos.get('symbol') == simbolo:
                            # Calcular PnL real
                            precio_entrada = float(pos.get('avgPrice', 0))
                            precio_actual = float(pos.get('markPrice', precio_entrada))
                            ...
                            # Determinar resultado basado en el PnL
                            if pnl_percent > 0:
                                resultado = 'TP'
                            else:
                                resultado = 'SL'
                            ...
                            if simbolo in self.operaciones_activas:
                                operacion_actual = self.operaciones_activas[simbolo]
                                datos_operacion = {
                                    'timestamp': datetime.now().isoformat(),
                                    'symbol': simbolo,
                                    'tipo': tipo,
                                    'precio_entrada': precio_entrada,
                                    'take_profit': operacion_actual.get('take_profit', 0),
                                    'stop_loss': operacion_actual.get('stop_loss', 0),
                                    'precio_salida': precio_actual,
                                    'resultado': resultado,
                                    'pnl_percent': pnl_percent,
                                    'duracion_minutos': 0,  # Se puede calcular si se guarda timestamp
                                    'angulo_tendencia': operacion_actual.get('angulo_tendencia', 0),
                                    'pearson': operacion_actual.get('pearson', 0),
                                    'r2_score': operacion_actual.get('r2_score', 0),
                                    'ancho_canal_relativo': operacion_actual.get('ancho_canal_relativo', 0),
                                    'ancho_canal_porcentual': operacion_actual.get('ancho_canal_porcentual', 0),
                                    'nivel_fuerza': operacion_actual.get('nivel_fuerza', 1),
                                    'timeframe_utilizado': operacion_actual.get('timeframe_utilizado', 'N/A'),
                                    'velas_utilizadas': operacion_actual.get('velas_utilizadas', 0),
                                    'stoch_k': operacion_actual.get('stoch_k', 0),
                                    'stoch_d': operacion_actual.get('stoch_d', 0),
                                    'breakout_usado': operacion_actual.get('breakout_usado', False),
                                    'operacion_ejecutada': operacion_actual.get('operacion_ejecutada', False)
                                }
            ...
            logger.error(f"❌ Error verificando operaciones reales en Bitget: {e}")
            logger.exception("Stack trace completo:")
            return []

    def obtener_datos_mercado_config(self, symbol, timeframe, num_velas):
        """Obtiene datos de mercado usando Bitget o Binance como fallback"""
        try:
            # Intentar con Bitget primero
            if self.bitget_client:
                ...
                if klines and len(klines) >= num_velas:
                    ...
                    precio_actual = cierres[0] if cierres else 0
                    return {
                        'cierres': cierres,
                        'maximos': maximos,
                        'minimos': minimos,
    ...
    def calcular_canal_regresion_config(self, datos_mercado, num_velas):
        """Calcula canal de regresión con configuración específica"""
        try:
            ...
            # Usar las últimas num_velas velas
            cierres = cierres[:num_velas]
            maximos = maximos[:num_velas]
            minimos = minimos[:num_velas]
            
            # Calcular regresión lineal
            x = list(range(len(cierres)))
            regresion_cierres = self.calcular_regresion_lineal(x, cierres)
            regresion_maximos = self.calcular_regresion_lineal(x, maximos)
            regresion_minimos = self.calcular_regresion_lineal(x, minimos)
            
            if not regresion_cierres or not regresion_maximos or not regresion_minimos:
                return None
            
            pendiente_cierres, intercepto_cierres = regresion_cierres
            pendiente_maximos, intercepto_maximos = regresion_maximos
            pendiente_minimos, intercepto_minimos = regresion_minimos
            
            # Calcular resistencia y soporte
            resistencia = pendiente_maximos * (len(cierres) - 1) + intercepto_maximos
            soporte = pendiente_minimos * (len(cierres) - 1) + intercepto_minimos
            
            # Calcular métricas de calidad
            pearson, angulo_grados = self.calcular_pearson_y_angulo(x, cierres)
            r2_score = self.calcular_r2(cierres, x, pendiente_cierres, intercepto_cierres)
            
            # Calcular ancho del canal
            precio_actual = cierres[0]
            ...
            return {
                ...
                'soporte': soporte,
                'pendiente_resistencia': pendiente_maximos,
                'pendiente_soporte': pendiente_minimos,
    ...
    def buscar_configuracion_optima_simbolo(self, symbol):
        """Busca la mejor configuración para un símbolo específico"""
        try:
            # Verificar si ya existe configuración óptima para este símbolo
            if symbol in self.config_optima_por_simbolo:
                ...
                if horas_desde_ultima < 2:  # Solo buscar cada 2 horas
                    return self.config_optima_por_simbolo[symbol]
            
            # Buscar la mejor configuración probando diferentes combinaciones
            mejor_config = None
            mejor_score = -1
            
            # Timeframes priorizados (cortos primero)
            timeframes = ['1m', '3m', '5m', '15m', '30m', '1h']
            velas_options = [80, 100, 120, 150, 200]
            
            for timeframe in timeframes:
                for num_velas in velas_options:
                    datos_mercado = self.obtener_datos_mercado_config(symbol, timeframe, num_velas)
                    if not datos_mercado:
                        continue
                    
                    info_canal = self.calcular_canal_regresion_config(datos_mercado, num_velas)
                    if not info_canal:
                        continue
                    
                    # Calcular score basado en calidad de la señal
                    score = 0
                    ...
                    # Bonus por Stochastic extremo
                    if info_canal['stoch_k'] <= 20 or info_canal['stoch_k'] >= 80:
                        score += 10
            ...
            if mejor_config:
                # Guardar configuración óptima para este símbolo
                self.config_optima_por_simbolo[symbol] = mejor_config
                self.ultima_busqueda_config[symbol] = datetime.now()
                
                logger.info(f"✅ Configuración óptima para {symbol}: {mejor_config}")
                return mejor_config
            
            return None
            
        except Exception as e:
            logger.error(f"❌ Error buscando configuración óptima para {symbol}: {e}")
            return None

    def detectar_breakout(self, symbol, info_canal, datos_mercado):
        """Detecta breakout en el canal de regresión"""
        try:
            ...
            # Evitar detectar breakouts repetidos muy seguidos
            if symbol in self.breakouts_detectados:
                ultimo_breakout = self.breakouts_detectados[symbol]
                tiempo_desde_ultimo = (datetime.now() - ultimo_breakout['timestamp']).total_seconds() / 3600
                if tiempo_desde_ultimo < 1:  # Al menos 1 hora entre breakouts del mismo símbolo
                    return None
            
            # Verificar condiciones para breakout alcista
            if angulo > 0 and precio_actual > resistencia:
                margen_breakout = resistencia * self.config.get('entry_margin', 0.001)
                if precio_actual >= resistencia + margen_breakout:
                    logger.info(f"🔼 BREAKOUT ALCISTA detectado en {symbol}")
                    logger.info(f"   Precio: {precio_actual:.8f}")
                    logger.info(f"   Resistencia: {resistencia:.8f}")
                    logger.info(f"   Margen: {margen_breakout:.8f}")
                    return 'LONG'
            
            # Verificar condiciones para breakout bajista
            if angulo < 0 and precio_actual < soporte:
                margen_breakout = soporte * self.config.get('entry_margin', 0.001)
                if precio_actual <= soporte - margen_breakout:
                    logger.info(f"🔽 BREAKOUT BAJISTA detectado en {symbol}")
                    logger.info(f"   Precio: {precio_actual:.8f}")
                    logger.info(f"   Soporte: {soporte:.8f}")
                    logger.info(f"   Margen: {margen_breakout:.8f}")
                    return 'SHORT'
            ...
            logger.error(f"❌ Error detectando breakout en {symbol}: {e}")
            return None

    def detectar_reentry(self, symbol, info_canal, datos_mercado):
        """Detecta reentry después de un breakout"""
        try:
            ...
            # Verificar tiempo máximo de espera (4 horas)
            tiempo_espera = (datetime.now() - breakout_info['timestamp']).total_seconds() / 3600
            if tiempo_espera > 4:
                logger.info(f"⏰ Timeout de espera para {symbol}, eliminando...")
                del self.esperando_reentry[symbol]
                return None
            
            # Para breakout alcista, buscar reentry bajista (SHORT)
            if tipo_breakout == 'LONG':
                margen_reentry = resistencia * self.config.get('entry_margin', 0.001)
                if precio_actual <= resistencia - margen_reentry:
                    logger.info(f"🔽 REENTRY BAJISTA detectado en {symbol} después de breakout alcista")
                    return 'SHORT'
            
            # Para breakout bajista, buscar reentry alcista (LONG)
            elif tipo_breakout == 'SHORT':
                margen_reentry = soporte * self.config.get('entry_margin', 0.001)
                if precio_actual >= soporte + margen_reentry:
                    logger.info(f"🔼 REENTRY ALCISTA detectado en {symbol} después de breakout bajista")
                    return 'LONG'
            
            return None
            
        except Exception as e:
            logger.error(f"❌ Error detectando reentry en {symbol}: {e}")
            return None

    def calcular_niveles_entrada(self, tipo_operacion, info_canal, precio_actual):
        """Calcula niveles de entrada, TP y SL para la operación"""
        try:
            if tipo_operacion == 'LONG':
                # Para LONG: entrada cerca del soporte, TP en resistencia, SL debajo del soporte
                entrada = info_canal['soporte'] * (1 + self.config.get('entry_margin', 0.001))
                take_profit = info_canal['resistencia']
                stop_loss = info_canal['soporte'] * (1 - self.config.get('entry_margin', 0.001))
            else:  # SHORT
                # Para SHORT: entrada cerca de la resistencia, TP en soporte, SL encima de la resistencia
                entrada = info_canal['resistencia'] * (1 - self.config.get('entry_margin', 0.001))
                take_profit = info_canal['soporte']
                stop_loss = info_canal['resistencia'] * (1 + self.config.get('entry_margin', 0.001))
            
            # Verificar ratio Risk/Reward mínimo
            riesgo = abs(entrada - stop_loss)
            beneficio = abs(take_profit - entrada)
            ratio_rr = beneficio / riesgo if riesgo > 0 else 0
            
            ratio_minimo = self.config.get('min_rr_ratio', 1.2)
            if ratio_rr < ratio_minimo:
                logger.warning(f"⚠️ Ratio R/R insuficiente para {tipo_operacion}: {ratio_rr:.2f} < {ratio_minimo}")
    ...
    def enviar_alerta_breakout(self, symbol, tipo_breakout, info_canal, datos_mercado, config_optima):
...
💰 <b>Precio Breakout:</b> {precio_actual:.8f}
...
💡 <b>Estrategia:</b> Esperando reentry para ejecutar operación
            ...
            if token and chat_ids:
                ...
                logger.info(f"✅ Alerta de breakout enviada para {symbol}")
                
        except Exception as e:
            logger.error(f"❌ Error enviando alerta de breakout para {symbol}: {e}")

    def escanear_mercado(self):
        """Escanea el mercado buscando señales de trading"""
        senales_encontradas = 0
        symbols = self.config.get('symbols', [])
        timeframes = self.config.get('timeframes', [])
        velas_options = self.config.get('velas_options', [])
        
        logger.info(f"🔍 Escaneando {len(symbols)} símbolos en {len(timeframes)} timeframes...")
...
🕒 {datos_operacion['timestamp']}
    ...
    def mostrar_resumen_operaciones(self):
        ...
        if self.bitget_client:
            print(f"   🤖 Bitget: ✅ Conectado (MARGEN AISLADO)")
    ...
    def iniciar(self):
        ...
        if self.bitget_client:
            ...
            if self.ejecutar_operaciones_automaticas:
                print(f"🤖 AUTO-TRADING: ✅ ACTIVADO (MARGEN AISLADO)")
            else:

...
# Configurar logging mejorado con más detalle
logging.basicConfig(
    level=logging.INFO, 
    stream=sys.stdout, 
    ...
    def buscar_mejores_parametros(self):
        ...
        if mejores_param:
            ...
            try:
                ...
                logger.warning("⚠ Error guardando mejores_parametros.json:", e)
        else:
...
class BitgetClient:
    def __init__(self, api_key, api_secret, passphrase):
        ...
        self.margin_mode = "isolated"  # Default a isolated margin
        logger.info(f"Cliente Bitget V2 inicializado con API Key: {api_key[:10]}...")
        logger.info(f"Modo de margen configurado: {self.margin_mode}")
    ...
    def _generate_signature(self, timestamp, method, request_path, body=''):
        ...
        try:
            ...
            logger.debug(f"Generando firma - Timestamp: {timestamp}, Method: {method}, Path: {request_path}, Body: {body_str}")
            ...
            if accounts:
                ...
                for account in accounts:
                    if account.get('marginCoin') == 'USDT':
                        ...
                        logger.info(f"✅ Balance USDT - Disponible: {available:.2f} | Bloqueado: {locked:.2f}")
                return True
            else:
    ...
    def get_account_info(self, product_type='USDT-FUTURES'):
        ...
        try:
            ...
            logger.debug(f"Obteniendo info de cuenta - ProductType: {product_type}")
            ...
            if response.status_code == 200:
                ...
                if data.get('code') == '00000':
                    ...
                    logger.info(f"✅ Obtenidas {len(accounts_data)} cuentas")
    ...
    def get_symbol_info(self, symbol):
        ...
        try:
            logger.debug(f"Obteniendo info del símbolo: {symbol}")
                ...
                if data.get('code') == '00000':
                    ...
                    for contract in contracts:
                        if contract.get('symbol') == symbol:
                            logger.info(f"✅ Símbolo encontrado en USDT-FUTURES: {symbol}")
                            return contract
            
            # Fallback a USDT-MIX
            logger.debug(f"Símbolo no encontrado en USDT-FUTURES, intentando USDT-MIX")
            ...
            if response.status_code == 200:
                ...
                if data.get('code') == '00000':
                    ...
                    for contract in contracts:
                        if contract.get('symbol') == symbol:
                            logger.info(f"✅ Símbolo encontrado en USDT-MIX: {symbol}")
                            return contract
            
            logger.error(f"❌ Símbolo {symbol} no encontrado en ningún producto")
            return None
    ...
    def place_order(self, symbol, side, order_type, size, price=None, 
        ...
        """Colocar orden de mercado o límite - CORREGIDO PARA API V2"""
        try:
            logger.info(f"🎯 Colocando orden - Símbolo: {symbol}, Side: {side}, Tipo: {order_type}, Tamaño: {size}")
            
            request_path = '/api/v2/mix/order/place-order'
            
            # Parámetros requeridos para API V2
            body = {
                'symbol': symbol,
                'productType': 'USDT-FUTURES',
                'marginMode': self.margin_mode,  # AGREGADO: parámetro requerido
                'marginCoin': 'USDT',
                'side': side,
            ...
            if client_order_id:
                body['clientOid'] = client_order_id  # CORREGIDO: clientOid en lugar de clientOrderId
            
            logger.debug(f"Cuerpo de la orden: {json.dumps(body, indent=2)}")
            ...
            if response.status_code == 200:
                ...
                if data.get('code') == '00000':
                    ...
                    logger.info(f"✅ Orden colocada exitosamente - ID: {order_id}, ClientOID: {client_oid}")
                    return order_data
                else:
                    error_msg = data.get('msg', 'Unknown error')
                    error_code = data.get('code', 'Unknown')
                    logger.error(f"❌ Error en orden: {error_code} - {error_msg}")
                    logger.error(f"❌ Cuerpo enviado: {json.dumps(body)}")
                    return None
            else:
                logger.error(f"❌ Error HTTP: {response.status_code} - {response.text}")
                logger.error(f"❌ Cuerpo enviado: {json.dumps(body)}")
            ...
            logger.error(f"❌ Parámetros: symbol={symbol}, side={side}, order_type={order_type}, size={size}")
            return None

    def place_plan_order(self, symbol, side, trigger_price, order_type, size, 
                         price=None, plan_type='normal_plan', trigger_type='fill_price'):
        """Colocar orden de plan (TP/SL) - CORREGIDO PARA API V2"""
        try:
            logger.info(f"📋 Colocando plan order - Símbolo: {symbol}, Side: {side}, Trigger: {trigger_price}")
            ...
            body = {
                ...
                'triggerType': trigger_type,  # AGREGADO: parámetro requerido
                'side': side,
                'orderType': order_type
            }
            ...
            logger.debug(f"Cuerpo del plan order: {json.dumps(body, indent=2)}")
            ...
            if response.status_code == 200:
                ...
                if data.get('code') == '00000':
                    ...
                    order_id = plan_data.get('orderId', 'N/A')
                    client_oid = plan_data.get('clientOid', 'N/A')
                    logger.info(f"✅ Plan order colocado exitosamente - ID: {order_id}, ClientOID: {client_oid}")
                    return plan_data
                else:
                    error_msg = data.get('msg', 'Unknown error')
                    error_code = data.get('code', 'Unknown')
            ...
            logger.error(f"❌ Parámetros: symbol={symbol}, side={side}, trigger_price={trigger_price}")
            return None

    def set_leverage(self, symbol, leverage, hold_side='long'):
        """Configurar apalancamiento"""
        try:
            logger.info(f"⚡ Configurando apalancamiento - Símbolo: {symbol}, Leverage: {leverage}x, Side: {hold_side}")
            ...
            logger.debug(f"Cuerpo set leverage: {json.dumps(body, indent=2)}")
            ...
            logger.info(f"Respuesta set leverage - Status: {response.status_code}")
            ...
            logger.error(f"❌ Parámetros: symbol={symbol}, leverage={leverage}, hold_side={hold_side}")
            return False

    def get_positions(self, symbol=None, product_type='USDT-FUTURES'):
        """Obtener posiciones abiertas"""
        try:
            logger.debug(f"Obteniendo posiciones - Símbolo: {symbol}, ProductType: {product_type}")
            ...
            if product_type == 'USDT-FUTURES':
                logger.debug("Intentando con USDT-MIX")
                return self.get_positions(symbol, 'USDT-MIX')
            
            logger.warning(f"No se pudieron obtener posiciones para {symbol}")
            return []
    ...
    def get_klines(self, symbol, interval='5m', limit=200):
        ...
        try:
            ...
            logger.debug(f"Obteniendo klines - Símbolo: {symbol}, Intervalo: {bitget_interval}, Límite: {limit}")
            ...
            if response.status_code == 200:
                ...
                if data.get('code') == '00000':
                    ...
                    logger.debug("Klines no disponibles en USDT-FUTURES, intentando USDT-MIX")
                    params['productType'] = 'USDT-MIX'
                    response = requests.get(
                        self.base_url + request_path,
                        params=params,
                    ...
                    if response.status_code == 200:
                        ...
                        if data.get('code') == '00000':
                            candles = data.get('data', [])
                            logger.debug(f"Obtenidas {len(candles)} velas de USDT-MIX")
                            return candles
...
def ejecutar_operacion_bitget(bitget_client, simbolo, tipo_operacion, capital_usd, leverage=20):
    ...
    try:
        ...
        logger.info("⚡ Paso 1/8: Configurando apalancamiento...")
        hold_side = 'long' if tipo_operacion == 'LONG' else 'short'
        leverage_ok = bitget_client.set_leverage(simbolo, leverage, hold_side)
        if not leverage_ok:
            logger.error("❌ Error configurando apalancamiento - abortando operación")
            return None
        time.sleep(0.5)
        
        # 3. Obtener precio actual
        logger.info("💰 Paso 2/8: Obteniendo precio actual...")
        ...
        logger.info("📊 Paso 3/8: Obteniendo información del símbolo...")
        ...
        logger.info("🧮 Paso 4/8: Calculando tamaño de posición...")
        ...
        if cantidad_contratos < min_trade_num:
            ...
            logger.warning(f"⚠️ Cantidad ajustada al mínimo: {min_trade_num}")
        ...
        logger.info("🎯 Paso 5/8: Calculando niveles TP/SL...")
        ...
        # 7. Abrir posición
        logger.info("📈 Paso 6/8: Abriendo posición...")
        ...
        if not orden_entrada:
            logger.error("❌ Error abriendo posición - orden de entrada falló")
            return None
        ...
        # 8. Colocar Stop Loss
        logger.info("🛑 Paso 7/8: Configurando Stop Loss...")
        ...
        logger.info("🎯 Paso 8/8: Configurando Take Profit...")
        ...
        logger.error(f"❌ Parámetros: simbolo={simbolo}, tipo={tipo_operacion}, capital={capital_usd}, leverage={leverage}")
        import traceback
...
class TradingBot:
    def __init__(self, config):
        ...
        if self.auto_optimize:
            try:
                ...
                logger.warning("⚠ Error en optimización automática:", e)
                parametros_optimizados = None
        ...
        for timeframe in timeframes:
            for num_velas in velas_options:
                try:
                    ...
                    logger.debug(f"   ⚠️ Error evaluando {timeframe}/{num_velas}: {e}")
                    continue
        if not mejor_config:
            for timeframe in timeframes:
                for num_velas in velas_options:
    ...
    def obtener_datos_mercado_config(self, simbolo, timeframe, num_velas):
        ...
        try:
            ...
            logger.warning(f"   ⚠️ Error obteniendo datos de Binance para {simbolo}: {e}")
            return None

    def calcular_canal_regresion_config(self, datos_mercado, num_velas):
        """Calcula canal de regresión con configuración específica"""
        try:
            maximos = datos_mercado['maximos'][-num_velas:]
            minimos = datos_mercado['minimos'][-num_velas:]
            cierres = datos_mercado['cierres'][-num_velas:]
            tiempos = list(range(len(cierres)))
            
            if len(maximos) < 14 or len(minimos) < 14 or len(cierres) < 14:
                return None
            
            # Calcular regresión lineal para resistencias (máximos)
            regresion_max = self.calcular_regresion_lineal(tiempos, maximos)
            if not regresion_max:
                return None
            
            pendiente_max, intercepto_max = regresion_max
            
            # Calcular regresión lineal para soportes (mínimos)
            regresion_min = self.calcular_regresion_lineal(tiempos, minimos)
            if not regresion_min:
                return None
            
            pendiente_min, intercepto_min = regresion_min
            
            # Calcular coeficiente de correlación y ángulo para la línea central
            pearson, angulo_grados = self.calcular_pearson_y_angulo(tiempos, cierres)
            if abs(pearson) < 0.3:
                return None
            
            # Calcular niveles de resistencia y soporte en el último punto
            ultimo_tiempo = tiempos[-1]
            resistencia = pendiente_max * ultimo_tiempo + intercepto_max
            soporte = pendiente_min * ultimo_tiempo + intercepto_min
            
            # Validar que resistencia > soporte
            if resistencia <= soporte:
                return None
            ...
            if ancho_canal_porcentual < self.config.get('min_channel_width_percent', 4.0):
                return None
            
            # Clasificar fuerza y dirección de la tendencia
            ...
            # Calcular R² score
            r2_score = self.calcular_r2(cierres, tiempos, pendiente_max, intercepto_max)
            
            # Calcular Stochastic
            stoch_k, stoch_d = self.calcular_stochastic(datos_mercado)
            
            # Calcular pendientes para las líneas de resistencia y soporte
            ...
            logger.warning(f"⚠️ Error calculando canal de regresión: {e}")
            return None
    ...
    def enviar_alerta_breakout(self, simbolo, tipo_breakout, info_canal, datos_mercado, config_optima):
        ...
        LÓGICA CORREGIDA:
        - BREAKOUT_LONG → Ruptura de RESISTENCIA en canal BAJISTA (precio rompe hacia arriba)
        - BREAKOUT_SHORT → Ruptura de SOPORTE en canal ALCISTA (precio rompe hacia abajo)
        """
        ...
        # CORREGIDO: Lógica correcta de mensajes de alerta
        if tipo_breakout == "BREAKOUT_LONG":
            # LONG: Ruptura de RESISTENCIA hacia arriba (canal bajista)
            emoji_principal = "🚀"
            ...
            # SHORT: Ruptura de SOPORTE hacia abajo (canal alcista)
            emoji_principal = "📉"
...
📍 {expectativa}
        ...
        if token and chat_ids:
            try:
                logger.debug(f"     📊 Generando gráfico de breakout para {simbolo}...")
                buf = self.generar_grafico_breakout(simbolo, info_canal, datos_mercado, tipo_breakout, config_optima)
                if buf:
                    logger.debug(f"     📨 Enviando alerta de breakout por Telegram...")
    ...
    def generar_grafico_breakout(self, simbolo, info_canal, datos_mercado, tipo_breakout, config_optima):
        ...
        try:
            ...
            # Agregar indicador de breakout
            df['Breakout'] = tipo_breakout
            ...
            if tipo_breakout == "BREAKOUT_LONG":
                ...
                apds.append(mpf.make_addplot([df['Close'].iloc[-1]], color='#ff6b6b', marker='^', markersize=100, panel=0))
            else:
                # Marcar ruptura de soporte
                apds.append(mpf.make_addplot([df['Close'].iloc[-1]], color='#4ecdc4', marker='v', markersize=100, panel=0))
            
            fig, axes = mpf.plot(df, type='candle', style='nightclouds',
                               title=f'BREAKOUT_{tipo_breakout} - {simbolo} | {config_optima["timeframe"]} | Bitget',
                               ylabel='Precio',
                               addplot=apds,
                               volume=False,
                               returnfig=True,
    ...
    def detectar_breakout(self, simbolo, info_canal, datos_mercado):
        ...
        try:
            ...
            pearson = abs(info_canal['coeficiente_pearson'])
            r2_score = info_canal['r2_score']
            
            # Verificar condiciones mínimas de calidad del canal
            if nivel_fuerza < 2 or pearson < 0.4 or r2_score < 0.4:
                return None
            
            # Lógica de detección de breakouts CORREGIDA
            margen = self.config.get('entry_margin', 0.001)
            
            # CORREGIDO: Lógica correcta de breakouts
            if precio_actual > resistencia * (1 + margen):
                # BREAKOUT_LONG: Ruptura de RESISTENCIA en canal bajista (precio rompe hacia arriba)
                if direccion_canal == "🔴 BAJISTA":
                    logger.debug(f"🎯 {simbolo} - BREAKOUT_LONG detectado (ruptura resistencia en canal bajista)")
                    return "BREAKOUT_LONG"
            
            elif precio_actual < soporte * (1 - margen):
                # BREAKOUT_SHORT: Ruptura de SOPORTE en canal alcista (precio rompe hacia abajo)
                if direccion_canal == "🟢 ALCISTA":
                    logger.debug(f"🎯 {simbolo} - BREAKOUT_SHORT detectado (ruptura soporte en canal alcista)")
                    return "BREAKOUT_SHORT"
            
            return None
            
        except Exception as e:
            logger.warning(f"⚠️ Error detectando breakout en {simbolo}: {e}")
            return None

    def detectar_reentry(self, simbolo, info_canal, datos_mercado):
        """Detecta reingreso al canal después de un breakout CON CONFIRMACIÓN STOCHASTIC"""
        try:
            ...
            # Verificar que no hayan pasado más de 30 minutos
            tiempo_desde_breakout = (datetime.now() - timestamp_breakout).total_seconds() / 60
            if tiempo_desde_breakout > 30:
                logger.debug(f"⏰ {simbolo} - Breakout expirado ({tiempo_desde_breakout:.1f} min)")
            ...
            # OBLIGATORIO: Confirmación con Stochastic
            if tipo_breakout == "BREAKOUT_LONG":
                # Reingreso para LONG: precio debe volver dentro del canal
                if soporte * (1 - margen) <= precio_actual <= resistencia * (1 + margen):
                    # OBLIGATORIO: Confirmar con Stochastic oversold para LONG
                    if stoch_k <= 30:
                        logger.info(f"✅ {simbolo} - Reentry LONG confirmado (Stoch K: {stoch_k:.1f} ≤ 30)")
                        return "LONG"
                    else:
                        logger.debug(f"⏸️ {simbolo} - Esperando confirmación Stoch para LONG (K: {stoch_k:.1f} > 30)")
            
            elif tipo_breakout == "BREAKOUT_SHORT":
                # Reingreso para SHORT: precio debe volver dentro del canal
                if soporte * (1 - margen) <= precio_actual <= resistencia * (1 + margen):
                    # OBLIGATORIO: Confirmar con Stochastic overbought para SHORT
                    if stoch_k >= 70:
                        logger.info(f"✅ {simbolo} - Reentry SHORT confirmado (Stoch K: {stoch_k:.1f} ≥ 70)")
                        return "SHORT"
                    else:
                        logger.debug(f"⏸️ {simbolo} - Esperando confirmación Stoch para SHORT (K: {stoch_k:.1f} < 70)")
            
            return None
            
        except Exception as e:
            logger.warning(f"⚠️ Error detectando reentry en {simbolo}: {e}")
            return None
    ...
    def escanear_mercado(self):
        ...
        logger.info(f"\n🔍 Escaneando {len(self.config.get('symbols', []))}ia: Breakout símbolos (Estrateg + Reentry)...")
...
🕒 {datos_operacion['timestamp']}
    ...
    def iniciar(self):
        ...
        if self.bitget_client:
            logger.info(f"🤖 BITGET: ✅ API V2 Conectada")

...
class BitgetClient:
    ...
    def generate_signature(self, method, request_path, body='', timestamp=''):
        """Genera firma para la API REST"""
        if not timestamp:
            timestamp = str(self.get_timestamp_ms())
    ...
    def send_request(self, method, endpoint, params=None, data=None):
        """Envía solicitud a la API REST de Bitget"""
        url = self.base_url + endpoint
        headers = {
            'Content-Type': 'application/json',
            'ACCESS-KEY': self.api_key,
            'ACCESS-TIMESTAMP': str(self.get_timestamp_ms()),
            'ACCESS-SIGN': self.generate_signature(method, endpoint, json.dumps(data) if data else '', str(self.get_timestamp_ms())),
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"❌ Error en API REST {method} {endpoint}: {response.status_code} - {response.text}")
                return None

            return response.json()

        except Exception as e:
            print(f"❌ Error en solicitud REST: {e}")
            return None

    def init_websocket(self):
        """Inicializa conexiones WebSocket"""
        try:
            # Conexión pública
            ...
            # Iniciar hilos para las conexiones
            thread_public = threading.Thread(target=self.ws_public.run_forever, daemon=True)
            thread_private = threading.Thread(target=self.ws_private.run_forever, daemon=True)
            thread_public.start()
            thread_private.start()

            # Iniciar hilo para ping
            thread_ping = threading.Thread(target=self.send_ping_loop, daemon=True)
            thread_ping.start()

        except Exception as e:
            print(f"❌ Error inicializando WebSocket: {e}")

    def on_public_open(self, ws):
        print("✅ Conexión WebSocket Pública establecida")

    def on_private_open(self, ws):
        print("✅ Conexión WebSocket Privada establecida")
        # Autenticar
        auth_msg = {
            "op": "login",
            "args": [{
                ...
                "timestamp": str(self.get_timestamp_s()),
                "sign": self.sign_message(f"{self.get_timestamp_s()}{self.api_key}{self.passphrase}")
            }]
        }
        ws.send(json.dumps(auth_msg))

    def on_public_message(self, ws, message):
        """Maneja mensajes de WebSocket público"""
        try:
            ...
            if 'data' in data and 'instId' in data['data'][0]:
                ...
                self.market_data[inst_id].append(data['data'][0])
                # Mantener solo los últimos 200 registros
                if len(self.market_data[inst_id]) > 200:
                    self.market_data[inst_id].pop(0)
        except Exception as e:
            pass  # Silenciar errores de procesamiento de mensajes

    def on_private_message(self, ws, message):
        """Maneja mensajes de WebSocket privado"""
        try:
            data = json.loads(message)
            # Procesar eventos de órdenes, posiciones, etc.
            pass
        except Exception as e:
            pass  # Silenciar errores de procesamiento de mensajes

    def on_public_error(self, ws, error):
        print(f"❌ Error WebSocket Público: {error}")

    def on_private_error(self, ws, error):
        print(f"❌ Error WebSocket Privado: {error}")

    def on_public_close(self, ws, close_status_code, close_msg):
        print("🔌 Conexión WebSocket Pública cerrada")

    def on_private_close(self, ws, close_status_code, close_msg):
        print("🔌 Conexión WebSocket Privada cerrada")

    def send_ping_loop(self):
        """Envía ping cada 30 segundos para mantener la conexión activa"""
        while True:
            ...
            try:
                ...
                if self.ws_private and self.ws_private.sock and self.ws_private.sock.connected:
                    self.ws_private.send(json.dumps({"op": "ping"}))
            except Exception:
                pass

    def subscribe_to_candles(self, symbols, interval='1m'):
        """Suscribirse a velas para símbolos específicos"""
        ...
        try:
            self.ws_public.send(json.dumps(subscription))
            return True
        except Exception as e:
            print(f"❌ Error suscribiendo a velas: {e}")
            return False

    def get_contract_info(self, symbol):
        """Obtiene información del contrato (precisión, tamaño mínimo, etc.)"""
        if symbol in self.symbol_info:
            return self.symbol_info[symbol]
        
        endpoint = f"/api/v2/mix/market/contracts?symbol={symbol}"
        result = self.send_request('GET', endpoint)
        
        if result and result.get('code') == '00000':
            contract = result.get('data', [{}])[0]
            info = {
                'minOrderSz': float(contract.get('minOrderSz', '0')),
                'maxOrderSz': float(contract.get('maxOrderSz', '0')),
                'priceTick': float(contract.get('priceTick', '0')),
                'lotSz': float(contract.get('lotSz', '0'))
            }
    ...
    def place_order(self, symbol, side, size, price=None, ord_type='market'):
    ...
    def get_market_data(self, symbol, interval='1m'):
        """Obtiene datos de mercado desde WebSocket"""
        if symbol not in self.market_data or len(self.market_data[symbol]) == 0:
            return None
        
        # Convertir datos WebSocket a formato compatible con el bot
        candles = []
        for candle_data in self.market_data[symbol]:
            # Formato: [open, high, low, close, volume, timestamp]
            candle = [
                float(candle_data['open']),
                float(candle_data['high']),
                float(candle_data['low']),
                float(candle_data['close']),
                float(candle_data['vol']),
                int(candle_data['ts'])
            ]
    ...
    def get_historical_candles(self, symbol, interval='1m', limit=200):
        """Obtiene velas históricas mediante REST API (inicialización)"""
        endpoint = f"/api/v2/mix/market/candles?symbol={symbol}&granularity={interval}&limit={limit}"
        result = self.send_request('GET', endpoint)
...
class TradingBot:
    def __init__(self, config):
        ...
        if bitget_api_key and bitget_secret_key and bitget_passphrase:
            self.bitget_client = BitgetClient(bitget_api_key, bitget_secret_key, bitget_passphrase)
            # Configurar modos y apalancamiento para todos los símbolos
            if self.bitget_client:
                for symbol in config.get('symbols', []):
                    self.bitget_client.set_margin_mode(symbol, 'isolated')
                    self.bitget_client.set_position_mode(symbol, 'one-way')
                    self.bitget_client.set_leverage(symbol, 10)
                    # Suscribirse a velas
                    self.bitget_client.subscribe_to_candles([symbol], '1m')
    ...
    def obtener_datos_mercado_config(self, simbolo, timeframe, num_velas):
        """Obtiene datos con configuración específica usando Bitget"""
        if not self.bitget_client:
            return None
            
        # Intentar obtener datos desde WebSocket primero
        datos = self.bitget_client.get_market_data(simbolo, timeframe)
        if not datos:
            # Si no hay datos en WebSocket, obtener datos históricos
            datos = self.bitget_client.get_historical_candles(simbolo, timeframe, num_velas)
            if not datos:
                return None
        ...
        # Tomar las últimas 'num_velas' velas
        datos = datos[-num_velas:]
        
        maximos = [float(candle[1]) for candle in datos]  # high
        minimos = [float(candle[2]) for candle in datos]  # low
        cierres = [float(candle[3]) for candle in datos]  # close
...
💡 <b>Estrategia:</b> BREAKOUT + REENTRY con confirmación Stochastic
        ...
        # Colocar la orden en Bitget
        self.ejecutar_orden_bitget(simbolo, tipo_operacion, precio_entrada, tp, sl)

    def ejecutar_orden_bitget(self, simbolo, tipo_operacion, precio_entrada, tp, sl):
        """Ejecuta la orden en Bitget"""
        if not self.bitget_client:
            print(f"❌ Cliente Bitget no inicializado, no se puede colocar orden para {simbolo}")
            return False
            
        # Obtener información del contrato
        contract_info = self.bitget_client.get_contract_info(simbolo)
        if not contract_info:
            print(f"❌ No se pudo obtener información del contrato para {simbolo}")
            return False
            
        # Calcular tamaño de la orden
        # Suponiendo que usamos 2 USDT como margen base
        margin = 2.0
        leverage = 10
        notional = margin * leverage
        size_raw = notional / precio_entrada
        lot_sz = contract_info['lotSz']
        price_tick = contract_info['priceTick']
        
        # Redondear tamaño y precio
        size = round(size_raw / lot_sz) * lot_sz
        price = round(precio_entrada / price_tick) * price_tick
        
        # Colocar orden de entrada
        side = 'buy' if tipo_operacion == 'LONG' else 'sell'
        order_result = self.bitget_client.place_order(simbolo, side, str(size), str(price), 'limit')
        
        if not order_result or order_result.get('code') != '00000':
            print(f"❌ Error colocando orden de entrada para {simbolo}: {order_result}")
            return False
            
        print(f"✅ Orden de entrada colocada para {simbolo}")
        
        # Colocar ordenes condicionales (TP/SL)
        # TP
        tp_side = 'sell' if tipo_operacion == 'LONG' else 'buy'
        tp_result = self.bitget_client.place_plan_order(
            simbolo, 
            'normal', 
            str(tp), 
            str(tp), 
        ...
        if not tp_result or tp_result.get('code') != '00000':
            print(f"❌ Error colocando orden TP para {simbolo}: {tp_result}")
        
        # SL
        sl_side = 'sell' if tipo_operacion == 'LONG' else 'buy'
        sl_result = self.bitget_client.place_plan_order(
            simbolo, 
            'normal', 
            str(sl), 
            str(sl), 
        ...
        if not sl_result or sl_result.get('code') != '00000':
            print(f"❌ Error colocando orden SL para {simbolo}: {sl_result}")
...
# ---------------------------
def crear_config_rapida():
    """Configuración rápida para iniciar el bot"""
    ...
    return {
        ...
        'telegram_chat_ids': ['2108159591'],
        'auto_optimize': True,
        'min_samples_optimizacion': 30,
        'reevaluacion_horas': 24,
        # Archivos en el mismo directorio del script
...
if __name__ == "__main__":
    print("
" + "=" * 80)
    print("🤖 BOT TRADING - ESTRATEGIA ROTURA + REINGRESO")
    print("=" * 80)
    print("
📋 Configuración:")
    print("   1️⃣  Detecta ROTURA ")
    print("   2️⃣  Espera REENTRADA ")
    print("   3️⃣  Confirma con STOCHASTIC (sobrecompra/sobreventa)")
    print("   4️⃣  Genera SEÑAL de entrada
")
    # Usar configuración rápida
    config = crear_config_rapida()
    print("✅ Configuración cargada")
    print(f"   📊 {len(config['symbols'])} símbolos")
    print(f"   ⏰ {len(config['timeframes'])} timeframes")
    print(f"   📏 Canal mínimo: {config['min_channel_width_percent']}%")
    print(f"   🔔 Telegram: {'Activado' if config['telegram_token'] else 'Desactivado'}
")
    confirmar = input("¿Iniciar el bot? (S/n): ").strip().lower()

...
# Configurar logging básico con más detalle
logging.basicConfig(
    level=logging.INFO, 
    stream=sys.stdout, 
...
# Configurar logging adicional para requests
logging.getLogger("requests").setLevel(logging.WARNING)
logging.getLogger("urllib3").setLevel(logging.WARNING)

logger.info("🚀 Bot Web Service iniciado")
logger.info(f"🐍 Python versión: {sys.version}")
logger.info(f"📅 Fecha/hora actual: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
class BitgetClient:
    def __init__(self, api_key, api_secret, passphrase):
        ...
        logger.info(f"🔧 Cliente Bitget inicializado")
    ...
    def _generate_signature(self, timestamp, method, request_path, body=''):
        ...
        try:
            ...
            # CORRECCIÓN: Formato correcto según documentación de Bitget
            # timestamp + method.toUpperCase() + requestPath + "?" + queryString + body
            message = timestamp + method.upper() + request_path + body_str
            
            logger.debug(f"Generando firma para: {message}")
    ...
    def verificar_credenciales(self):
        ...
        try:
            ...
            if not self.api_key or not self.api_secret or not self.passphrase:
                ...
                logger.error(f"   API Secret: {'✓' if self.api_secret else '✗'}")
                logger.error(f"   Passphrase: {'✓' if self.passphrase else '✗'}")
                return False
            
            logger.info(f"✓ API Key presente: {self.api_key[:10]}...")
            logger.info(f"✓ API Secret presente: {'*' * len(self.api_secret)}")
            logger.info(f"✓ Passphrase presente: {'*' * len(self.passphrase)}")
            ...
            if accounts:
                ...
                for account in accounts:
                    if account.get('marginCoin') == 'USDT':
                        ...
                        logger.info(f"   💰 Equity total: {equity:.2f} USDT")
                        logger.info(f"   🔒 Balance bloqueado: {float(account.get('locked', 0)):.2f} USDT")
                return True
            else:
                ...
                logger.error("   - Cuenta suspendida")
    ...
    def get_account_info(self, product_type='USDT-FUTURES'):
        ...
        try:
            # CORRECCIÓN: URL y parámetros correctos según documentación oficial
            request_path = '/api/v2/mix/account/accounts'
            
            # CORRECCIÓN: productType debe ser exactamente 'USDT-FUTURES' (no 'USDT-FUTURES')
            params = {'productType': 'USDT-FUTURES'}
            
            # Query string sin marginCoin para evitar errores
            query_string = f"?productType=USDT-FUTURES"
            full_request_path = request_path + query_string
            
            logger.info(f"Solicitando información de cuenta con productType=USDT-FUTURES")
            ...
            logger.debug(f"Respuesta completa: {response.text[:500]}")
            
            if response.status_code == 200:
                data = response.json()
                if data.get('code') == '00000':
                    logger.info("✓ Información de cuenta obtenida exitosamente")
                    ...
                    # MEJORADO: Manejo específico de errores con retry
                    if error_code == '40020':
                        logger.error("❌ Error 40020: Parámetro productType incorrecto")
                        logger.info("Intentando con productType='COIN-FUTURES'...")
                        return self.get_account_info('COIN-FUTURES')
                    elif error_code == '40009':
                        logger.error("❌ Error 40009: Error de firma en credenciales")
                        logger.info("Verificando credenciales...")
                    elif error_code == '40010':
                        logger.error("❌ Error 40010: Request timeout")
                        logger.info("Reintentando en 2 segundos...")
                        time.sleep(2)
                        return self.get_account_info(product_type)
            else:
            ...
            logger.error(f"❌ Timeout en get_account_info: {e}")
    ...
    def place_order(self, symbol, side, order_type, size, price=None, 
        ...
        try:
            ...
            # CORRECCIÓN: Parámetros correctos según documentación oficial
            body = {
                'symbol': symbol,
            ...
            logger.info(f"Colocando orden: {symbol} - {side} - {order_type} - {size}")
            ...
            logger.debug(f"Respuesta orden - Status: {response.status_code}")
            logger.debug(f"Respuesta orden - Body: {response.text}")
            ...
            if response.status_code == 200:
                ...
                if data.get('code') == '00000':
                    ...
                    if error_code == '40009':
                        logger.error("❌ Error 40009: Signature error en place_order")
                    elif error_code == '40020':
                        logger.error("❌ Error 40020: Parámetros incorrectos en place_order")
                    elif error_code == '40017':
                        logger.error("❌ Error 40017: Parameter verification failed")
                    elif error_code == '40021':
                        logger.error("❌ Error 40021: User disable withdraw")
                    elif error_code == '40022':
                        logger.error("❌ Error 40022: Account business restricted")
                    elif error_code == '40023':
                        logger.error("❌ Error 40023: API service restricted")
                    elif error_code == '40024':
                        logger.error("❌ Error 40024: Order type not supported")
                    elif error_code == '40025':
                        logger.error("❌ Error 40025: Position mode not supported")
                    elif error_code == '40026':
                        logger.error("❌ Error 40026: Account is frozen")
                    elif error_code == '40027':
                        logger.error("❌ Error 40027: Order amount too small")
                    elif error_code == '40028':
                        logger.error("❌ Error 40028: Order amount too large")
                    elif error_code == '40029':
                        logger.error("❌ Error 40029: Order price out of range")
                    elif error_code == '40030':
                        logger.error("❌ Error 40030: Insufficient balance")
            ...
            logger.error(f"❌ Error de conexión en place_order: {e}")
            return None
        except requests.exceptions.Timeout as e:
            logger.error(f"❌ Timeout en place_order: {e}")
            return None
        except Exception as e:
    ...
    def set_leverage(self, symbol, leverage, hold_side='long'):
        ...
        try:
            ...
            logger.debug(f"Respuesta set_leverage - Status: {response.status_code}")
            logger.debug(f"Respuesta set_leverage - Body: {response.text}")
            ...
            if response.status_code == 200:
                ...
                if data.get('code') == '00000':
                    ...
                    # MEJORADO: Manejo específico de errores
                    if error_code == '40009':
                        logger.error("❌ Error 40009: Signature error en set_leverage")
                    elif error_code == '40020':
                        logger.error("❌ Error 40020: Parámetros incorrectos en set_leverage")
                    elif error_code == '40021':
                        logger.error("❌ Error 40021: Leverage no permitido para esta cuenta")
                    elif error_code == '40022':
                        logger.error("❌ Error 40022: Account business restricted")
                    elif error_code == '40023':
                        logger.error("❌ Error 40023: API service restricted")
            ...
            logger.error(f"❌ Error de conexión en set_leverage: {e}")
            return False
        except requests.exceptions.Timeout as e:
            logger.error(f"❌ Timeout en set_leverage: {e}")
            return False
        except Exception as e:
    ...
    def get_positions(self, symbol=None, product_type='USDT-FUTURES'):
        ...
        try:
            ...
            logger.debug(f"Respuesta get_positions - Status: {response.status_code}")
            
            if response.status_code == 200:
                data = response.json()
                if data.get('code') == '00000':
                    positions = data.get('data', [])
                    logger.debug(f"✓ Obtenidas {len(positions)} posiciones")
                    return positions
                else:
                    error_code = data.get('code', 'Unknown')
                    error_msg = data.get('msg', 'Unknown error')
                    ...
                    # MEJORADO: Retry con diferentes productType
                    if error_code == '40020':
                        logger.info("Reintentando con productType='COIN-FUTURES'...")
                        return self.get_positions(symbol, 'COIN-FUTURES')
            
            logger.warning(f"⚠️ No se pudieron obtener posiciones: {response.status_code} - {response.text}")
            return []
            
        except requests.exceptions.ConnectionError as e:
            logger.error(f"❌ Error de conexión en get_positions: {e}")
            return []
        except requests.exceptions.Timeout as e:
            logger.error(f"❌ Timeout en get_positions: {e}")
            return []
        except Exception as e:
    ...
    def get_klines(self, symbol, interval='5m', limit=200):
        ...
        try:
            ...
            # CORRECCIÓN: Parámetros correctos según documentación
            params = {
                'symbol': symbol,
                'productType': 'USDT-FUTURES',
            ...
            logger.debug(f"Obteniendo klines para {symbol} con granularity={bitget_interval}, limit={limit}")
            ...
            logger.debug(f"Klines response status: {response.status_code}")
            ...
            if response.status_code == 200:
                ...
                if data.get('code') == '00000':
                    ...
                    if error_code == '40020':
                        ...
                        logger.error("❌ Error 40009: Signature error en get_klines")
                    elif error_code == '40010':
                        logger.error("❌ Error 40010: Timeout en get_klines")
...
def ejecutar_operacion_bitget(bitget_client, simbolo, tipo_operacion, capital_usd, leverage=20):
    ...
    try:
        ...
        if cantidad_contratos < min_trade_num:
            ...
            logger.warning(f"⚠️ Cantidad ajustada al mínimo: {cantidad_contratos}")
        ...
        logger.info(f"   🛑 Stop Loss: {stop_loss:.8f} (-2%)")
        logger.info(f"   🎯 Take Profit: {take_profit:.8f} (+4%)")
        ...
        logger.error(f"❌ Error de conexión ejecutando operación: {e}")
        return None
    except requests.exceptions.Timeout as e:
        logger.error(f"❌ Timeout ejecutando operación: {e}")
        return None
    except Exception as e:
    ...
    def __init__(self, config):
        ...
        if config.get('bitget_api_key') and config.get('bitget_api_secret') and config.get('bitget_passphrase'):
            ...
            if self.bitget_client.verificar_credenciales():
                ...
                logger.info(f"🤖 Operaciones automáticas: {'Activadas' if self.ejecutar_operaciones_automaticas else 'Desactivadas'}")
                logger.info(f"💰 Capital por operación: ${self.capital_por_operacion}")
                logger.info(f"⚡ Apalancamiento por defecto: {self.leverage_por_defecto}x")
            else:
                logger.warning("⚠️ No se pudieron verificar las credenciales de Bitget")
                logger.warning("   El bot funcionará solo en modo de señales")
        else:
            logger.info("ℹ️ Credenciales de Bitget no encontradas en variables de entorno")
            logger.info("   Variables requeridas:")
            logger.info("   - BITGET_API_KEY")
            logger.info("   - BITGET_SECRET_KEY") 
            logger.info("   - BITGET_PASSPHRASE")
            logger.info("   El bot funcionará solo en modo de señales")
...
📍 {expectativa}
    ...
    def generar_grafico_breakout(self, simbolo, info_canal, datos_mercado, tipo_breakout, config_optima):
        ...
        try:
            ...
            # Dibujar canales y marcar ruptura
            ...
            fig, axes = mpf.plot(df, type='candle', style='nightclouds',
                               title=f'{simbolo} - BREAKOUT {tipo_breakout} - {config_optima["timeframe"]}',
                               ylabel='Precio',
                               addplot=[
                                   mpf.make_addplot(df['Resistencia'], color='#ff4444', linestyle='--', width=2),
                                   mpf.make_addplot(df['Soporte'], color='#44ff44', linestyle='--', width=2),
                                   mpf.make_addplot([precio_breakout] * len(df), color='#ffff44', linestyle='-', width=3)
                               ],
                               volume=False,
                               returnfig=True,
                               figsize=(14, 10))
    ...
    def detectar_breakout(self, simbolo, info_canal, datos_mercado):
        """Detecta breakouts según la estrategia Breakout + Reentry"""
        precio_actual = datos_mercado['precio_actual']
        resistencia = info_canal['resistencia']
        soporte = info_canal['soporte']
        direccion = info_canal['direccion']
        
        # Solo detectar si la fuerza de la tendencia es suficiente
        if info_canal['nivel_fuerza'] < 2:
            return None
        
        # Detectar ruptura de soporte en canal bajista (oportunidad LONG)
        if precio_actual < soporte and direccion == "🔴 BAJISTA":
            # Verificar si es un breakout válido (no una mecha falsa)
            if len(datos_mercado['cierres']) >= 2:
                precio_anterior = datos_mercado['cierres'][-2]
                if precio_anterior >= soporte * 0.999:  # Estaba dentro o cerca del canal
                    logger.info(f"🔴 {simbolo} - BREAKOUT_LONG detectado (ruptura de soporte en canal bajista)")
                    return "BREAKOUT_LONG"
        
        # Detectar ruptura de resistencia en canal alcista (oportunidad SHORT)
        elif precio_actual > resistencia and direccion == "🟢 ALCISTA":
            # Verificar si es un breakout válido (no una mecha falsa)
            if len(datos_mercado['cierres']) >= 2:
                precio_anterior = datos_mercado['cierres'][-2]
                if precio_anterior <= resistencia * 1.001:  # Estaba dentro o cerca del canal
                    logger.info(f"🟢 {simbolo} - BREAKOUT_SHORT detectado (ruptura de resistencia en canal alcista)")
                    return "BREAKOUT_SHORT"
        
        return None

    def detectar_reentry(self, simbolo, info_canal, datos_mercado):
        """Detecta reentry al canal después de un breakout"""
        if simbolo not in self.esperando_reentry:
            return None
        
        info_breakout = self.esperando_reentry[simbolo]
        tiempo_desde_breakout = (datetime.now() - info_breakout['timestamp']).total_seconds() / 60
        
        # Timeout: máximo 30 minutos para reentry
        if tiempo_desde_breakout > 30:
            logger.info(f"⏰ {simbolo} - Timeout de reentry ({tiempo_desde_breakout:.1f} min)")
        ...
        # Verificar si el precio regresó al canal
        if soporte <= precio_actual <= resistencia:
            ...
            if tipo_breakout == "BREAKOUT_LONG":
                # Para LONG: Stochastic debe estar en oversold (< 30)
                if stoch_k <= 30:
                    logger.info(f"🚀 {simbolo} - Reentry LONG confirmado (Stoch: {stoch_k:.1f})")
                    return "LONG"
                else:
                    logger.info(f"⏳ {simbolo} - Esperando confirmación Stochastic para LONG (actual: {stoch_k:.1f})")
            elif tipo_breakout == "BREAKOUT_SHORT":
                # Para SHORT: Stochastic debe estar en overbought (> 70)
                if stoch_k >= 70:
                    logger.info(f"📉 {simbolo} - Reentry SHORT confirmado (Stoch: {stoch_k:.1f})")
                    return "SHORT"
                else:
                    logger.info(f"⏳ {simbolo} - Esperando confirmación Stochastic para SHORT (actual: {stoch_k:.1f})")
...
🕒 {datos_operacion['timestamp']}
    ...
    def ejecutar_analisis(self):
        try:
            ...
            # Reoptimización automática (10% de probabilidad)
            if random.random() < 0.1:
                logger.debug("🔄 Ejecutando reoptimización automática...")
            ...
            if cierres:
                logger.info(f"📊 Operaciones cerradas en este ciclo: {', '.join(cierres)}")
            
            # Escaneo principal del mercado
            logger.debug("🔍 Iniciando escaneo del mercado...")
            nuevas_senales = self.escanear_mercado()
            
            # Guardar estado
            self.guardar_estado()
            
            logger.debug(f"✅ Ciclo de análisis completado. Nuevas señales: {nuevas_senales}")
            return nuevas_senales
            
        except requests.exceptions.ConnectionError as e:
            logger.error(f"❌ Error de conexión en ejecutar_analisis: {e}")
            time.sleep(5)  # Esperar antes de reintentar
            return 0
        except requests.exceptions.Timeout as e:
            logger.error(f"❌ Timeout en ejecutar_analisis: {e}")
            time.sleep(10)  # Esperar más tiempo en caso de timeout
            return 0
        except Exception as e:
            logger.error(f"❌ Error crítico en ejecutar_analisis: {e}")
            logger.error(f"   Tipo de error: {type(e).__name__}")
            logger.error(f"   Detalles: {str(e)}")
            time.sleep(30)  # Esperar más tiempo en caso de error crítico
            return 0
    ...
    def mostrar_resumen_operaciones(self):
        ...
        if self.operaciones_activas:
            print(f"   📋 Detalle de operaciones activas:")
            ...
            print(f"   ℹ️ No hay operaciones activas")
            
        if self.esperando_reentry:
            print(f"   ⏳ Símbolos esperando reentry:")
            for simbolo, info in self.esperando_reentry.items():
                tiempo_espera = (datetime.now() - info['timestamp']).total_seconds() / 60
                print(f"      • {simbolo} - {info['tipo']} - Esperando {tiempo_espera:.1f} min")
    ...
    def iniciar(self):
        ...
        if self.bitget_client:
            ...
            if self.ejecutar_operaciones_automaticas:
                print(f"🤖 AUTO-TRADING: ✅ ACTIVADO - Las operaciones se ejecutarán automáticamente")
                print(f"⚠️ IMPORTANTE: Verifique que las credenciales y configuraciones sean correctas")
            else:
                print(f"🤖 AUTO-TRADING: ❌ Desactivado - Solo se enviarán señales")
        else:
            print(f"🤖 BITGET: ❌ No configurado (solo señales por Telegram)")
            print(f"ℹ️ Para habilitar auto-trading, configure las variables:")
            print(f"   - BITGET_API_KEY")
            print(f"   - BITGET_SECRET_KEY")
            print(f"   - BITGET_PASSPHRASE")
            print(f"   - EJECUTAR_OPERACIONES_AUTOMATICAS=true")
        
        print(f"📱 TELEGRAM: {'✅ Configurado' if self.config.get('telegram_token') and self.config.get('telegram_chat_ids') else '❌ No configurado'}")
        print("=" * 70)
        print("\n🚀 INICIANDO BOT...")
        print("📊 El bot escaneará el mercado cada", self.config.get('scan_interval_minutes', 1), "minutos")
        print("⏹️  Presione Ctrl+C para detener el bot")
        print("=" * 70)
...
if __name__ == '__main__':
    logger.info("🚀 INICIANDO BOT WEB SERVICE")
    logger.info("=" * 50)
    ...
    try:
        ...
        logger.warning(f"⚠️ Error configurando webhook de Telegram: {e}")
    
    logger.info("🌐 Iniciando servidor Flask...")
    logger.info("📱 Endpoints disponibles:")
    logger.info("   GET  / - Estado del servicio")
    logger.info("   POST /webhook - Webhook de Telegram")
    logger.info("=" * 50)
